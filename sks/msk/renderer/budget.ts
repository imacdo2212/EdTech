...
// Hard renderer budget (char proxy for tokens)
const RENDER_BUDGET = {
  maxChars: 1200
};
...
/**
 * INVARIANTS:
 * - Renderer expresses only
 * - Renderer output is schema-validated + bounded
 * - MSK owns stage resolution
 */
export function mskRoute(
  ...
  const rendered = guardRenderedOutput(rawRendered, RENDER_BUDGET);
